...
### Three generations of mod, all shipped under `mods/`

1. **Expanded-archive mods** (`90s_luigi_expanded`) — the newest approach; the only one that allows replacement meshes with MORE vertices than the vanilla allocation. `res/build_expanded_global.py` relocates each touched slot's whole B005 chunk (vanilla bytes + appended new streams) into the freed vanilla B005 regions of the relocated/mirrored slots — section 54 NEVER changes size — and rewrites the B005/mirror records in `global.patch`'s 13 chunk-table copies (the table the game actually reads). `res/package_expanded_global.py` packages it; `installer/install_expanded_global.py` is the template (fixed-size sections only). **Read `expand_archive/EXPANSION_WORKFLOW.md` before touching this** — it has the full recipe, the format reference, and the critical rule: the 1.4 update owns the address space past every vanilla section end (patch-table B005s run to `0xBA63260`, 8.9MB past section 54), so growing a section overlays the update's relocated models and explodes them in-game.
2. **Compact section-delta mods** (`mario_playable`, `*_hd_playable_face_works`) — fixed-layout approach. `res/package_mario_global_sections.py` diffs a validated build against the clean archive per decompressed section, emits `MGP0` (or zlib-wrapped `MGPZ`) range-replacement deltas plus a SHA-256 `sections.json`, and copies the installer template. Ships as `install.py` + `sections.json` + `global_section_*.bin` + `README.txt` and needs no source assets.
3. **Dynamic slot-clone mods** (`N64_*`, `Mummigi_playable`, `Paleontoluigist_playable`, `groovigi_playable`, `gooigi_playable`) — earliest approach. Ships `lm3_costume_mod_builder.py` + `lm3_slot_swap.py` + `util.py` and rebuilds the archive on the user's machine by cloning a Persistent costume slot over the Story slots. `dynamic_installer.run(...)` is the shared driver for the newer ones.
