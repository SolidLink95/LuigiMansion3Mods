...
from util import decode_archive, texture_records
...
GLOBAL = Path("global.dict")
PERSISTENT = Path("Scarescraper/Persistent.dict")
OUTPUT = Path("romfs")
...
# Optional deterministic source-texture -> owned Global-texture assignment.
TEXTURE_MAPPING_OVERRIDE = {}
...
def main():
    ...
    mapping = dict(TEXTURE_MAPPING_OVERRIDE)
    if mapping:
        if set(mapping) != needed_textures:
            raise ValueError("texture mapping override does not cover exactly the needed textures")
        for source_hash, target_hash in mapping.items():
            if target_hash not in owned_pool:
                raise ValueError(f"texture mapping target {target_hash:08X} is not owned")
            if source_textures[source_hash][1].size > global_textures[target_hash][1].size:
                raise ValueError(f"texture mapping target {target_hash:08X} is too small")
    else:
