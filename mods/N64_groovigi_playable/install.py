...
def main() -> int:
    try:
        ...
        builder.OUTPUT = PACKAGE_DIR / "N64_groovigi_playable" / "romfs"
        builder.TARGET_SOURCE_PAIRS = tuple((target, 52) for target in (27, 28, 29, 30))
        builder.TARGETS = (27, 28, 29, 30)
        builder.SOURCES = (52,)
        builder.main()
    except (KeyError, OSError, ValueError, AssertionError) as error:
        return fail(str(error))
    print("created N64_groovigi_playable in " + str((PACKAGE_DIR / "N64_groovigi_playable" / "romfs").resolve()))
    return 0
