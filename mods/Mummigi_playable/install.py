"""Build the high-detail Mummigi mod dynamically from a clean LM3 ROMFS."""
...
def main() -> int:
    try:
        ...
        builder.OUTPUT = PACKAGE_DIR / "Mummigi_playable" / "romfs"
        builder.TARGET_SOURCE_PAIRS = tuple(
            (target, 54) for target in (27, 28, 29, 30)
        )
    ...
    print(
        "created Mummigi_playable in "
        + str((PACKAGE_DIR / "Mummigi_playable" / "romfs").resolve())
    )
    return 0
